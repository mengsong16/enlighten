...
from enlighten.utils.image_utils import (
    center_crop,
...
class ResizeShortestEdge(ObservationTransformer):
    ...
    @classmethod
    def from_config(cls, config):
        return cls(int(config.get("obs_transforms_resize_shortest_edge_size")))
...
class CenterCropper(ObservationTransformer):
    ...
    def from_config(cls, config):
        return cls(
            (
                int(config.get("obs_transforms_center_cropper_height")),
                int(config.get("obs_transforms_center_cropper_width")),
            )
        )
...
class CubeMap2Equirect(ProjectionTransformer):
    ...
    @classmethod
    def from_config(cls, config):
        if config.get("obs_transforms_cube2eq_target_uuids") is not None:
            # Optional Config Value to specify target UUID
            target_uuids = list(config.get("obs_transforms_cube2eq_target_uuids"))
        else:
            target_uuids = None
        return cls(
            list(config.get("obs_transforms_cube2eq_sensor_uuids")),
            eq_shape=(
                int(config.get("obs_transforms_cube2eq_height")),
                int(config.get("obs_transforms_cube2eq_width")),
            ),
...
class CubeMap2Fisheye(ProjectionTransformer):
    ...
    @classmethod
    def from_config(cls, config):
        if config.get("obs_transforms_cube2fish_target_uuids") is not None:
            # Optional Config Value to specify target UUID
            target_uuids = list(config.get("obs_transforms_cube2fish_target_uuids"))
        else:
            target_uuids = None
        
        return cls(
            list(config.get("obs_transforms_cube2fish_sensor_uuids")),
            fish_shape=(
                int(config.get("obs_transforms_cube2fish_height")),
                int(config.get("obs_transforms_cube2fish_width")),
            ),
            fish_fov=float(config.get("obs_transforms_cube2fish_fov")),
            fish_params=config.get("obs_transforms_cube2fish_params"),
            target_uuids=target_uuids,
        )
...
class Equirect2CubeMap(ProjectionTransformer):
    ...
    # eq2cube_config: obs_transforms_eq2cube
    @classmethod
    def from_config(cls, config):
        if config.get("obs_transforms_eq2cube_target_uuids") is not None:
            # Optional Config Value to specify target UUID
            target_uuids = list(config.get("obs_transforms_eq2cube_target_uuids"))
        else:
            target_uuids = None
        return cls(
            list(config.get("obs_transforms_eq2cube_sensor_uuids")),
            img_shape=(
                int(config.get("obs_transforms_eq2cube_height")),
                int(config.get("obs_transforms_eq2cube_width")),
            ),
            target_uuids=target_uuids,
        )


def get_active_obs_transforms(config) -> List[ObservationTransformer]:
    active_obs_transforms = []
    if len(list(config.get("enabled_transforms"))) > 0:
        obs_transform_names = (
            config.get("enabled_transforms")
        )
        for obs_transform_name in obs_transform_names:
            # __name__ is current module
            obs_trans_cls = getattr(
                sys.modules[__name__], obs_transform_name
            )
